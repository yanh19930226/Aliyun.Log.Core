...
namespace Aliyun.Log.Core.Client.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            if (string.IsNullOrEmpty(QueryTimeSpan) ||  pagesize == 0 || page == 0)
            {
                return Json("");
            }
            ...
            //获取总条数
            long count = 0;
            var res = await Startup.client.GetHistogramsAsync(_options.Value.LogstoreName, new DateTimeOffset(from), new DateTimeOffset(to));
